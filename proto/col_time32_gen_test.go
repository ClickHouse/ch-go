...
func TestColTime32_DecodeColumn(t *testing.T) {
	t.Parallel()
	const rows = 50
	var data ColTime32
	for i := 0; i < rows; i++ {
	...
	t.Run("Golden", func(t *testing.T) {
		t.Parallel()
		gold.Bytes(t, buf.Buf, "col_time32")
	})
...
func BenchmarkColTime32_DecodeColumn(b *testing.B) {
	const rows = 1_000
	var data ColTime32
	for i := 0; i < rows; i++ {
		data.Append(Time32(i))
	}
...
func BenchmarkColTime32_EncodeColumn(b *testing.B) {
	const rows = 1_000
	var data ColTime32
	for i := 0; i < rows; i++ {
		data.Append(Time32(i))
