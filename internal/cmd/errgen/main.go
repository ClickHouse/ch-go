...
func main() {
	...
	flag.StringVar(&arg.Name, "f", "error_gen.go", "file name")
	flag.Parse()

	acronym := map[string]bool{}
	...
	s := bufio.NewScanner(strings.NewReader(errorValues))
	for s.Scan() {
		...
		fmt.Println(b.String(), "Error", "=", value)
	}
	fmt.Println(")")
}
